...
local function callbackSecureCommCpp(id, responseCode, responseBuffer, responseHeaders, responseData, postData, curlCode, curlErrorStr, effectiveURL)
  ...
  if not r then
	...
	return false -- added this cause this function was throwing a nil error, hope it doesnt break something else
  end
  ...
  if type(responseData) == 'table' and tableSize(responseData) > 0 then
...
local function onUIOnlineMessageHide(uid)
  ...
  settings.setValue("OnlineHiddenMessageIDs", join(hidden_ids, ','))
  settings.save()
end
